...
load("hMat_PedNH_CCmat_fndrMrkData19.rdata")
yr<-"2019"

### !!!!
dataNHpi<-dataNHpi20_C  
dataNHim<-dataNHim20_C
load("hMat_PedNH_CCmat_fndrMrkData20.rdata")
yr<-"2020"
...
dataNHpi$isSelf <- isSelf 
  str(dataNHpi)
# Experimental design variables should be factors
for (col in c("Year","plotNo", "femaPar", "femaParLoc", "malePar", "maleParLoc", "line", "block", "date", "popChk", "withinLoc", "isSelf")) 
...
dataNHpi$GrowDays<-as.factor(dataNHpi$GrowDays)

### Analyze GOM vs SNE separately !!!!!!!
### one analysis rm Photo score =0, another one with rm anything <2
### GrowDays confounded with Year

msX <- model.matrix( ~ line+block+popChk, data=dataNHpi)  ### !!!! nested effects???? DID NOT give the full Ranks of X
...
#msX2 <- msX[, apply(msX, 2, function(v) !sd(v) == 0)]
...
# 1. Density Blades covariate
  ...
  msXdb <- model.matrix( ~ densityBlades + line + block + popChk, data=dataNHpi) ### !!!! Change???
...
#CrossedSP<-read.csv("Crossed_WildSP_2019_2020.csv",sep=",",header=TRUE)
  ...
  # for (col in c("plotNo", "subPlot", "plotSub"))  ## RM subPlot !!!! No subPlot this year ?????
  #   dataNHim[,col] <- as.factor(dataNHim[,col])
 #### dataNHim$plotNo2 <- factor(dataNHim$plotNo, levels=levels(dataNHpi$plotNo))
...
################## Does the order of plot in msXim/dataNHim matter????????
  ...
  names(h2hMatim) <- c("bladeLength", "bladeMaxWidth",  "bladeThickness", "stipeLength", "stipeDiameter") #"bladeWid10cm",
  
    h2hMatim
   
    # bladeLength  bladeMaxWidth bladeThickness    stipeLength  stipeDiameter 
    # 0.007576507    0.105867294    0.384318662    0.274271740    0.463622128 
    
    #bladeLength  bladeMaxWidth bladeThickness    stipeLength  stipeDiameter 
    #0.17470230     0.07852417     0.76631333     0.55484099     0.81967639 
...
#####################
  ...
  #msOutBW10h$u,
  allBLUPs <- cbind(msOutWWPh$u, msOutDWPMh$u, msOutPDWh$u, msOutDBh$u, msOutDevelh$u, msOutBLh$u, msOutBMWh$u,  msOutBTh$u, msOutSLh$u, msOutSDh$u)
  colnames(allBLUPs) <- c("WWP", "DWpM", "PDW", "BDns", "SPdv", "BLen", "BMax",  "BThk", "SLen", "SDia")
  
  pdf("/Users/maohuang/Desktop/Kelp/2020_2019_Phenotypic_Data/Phenotypic_Analysis/CorrPlot_2020.pdf")
